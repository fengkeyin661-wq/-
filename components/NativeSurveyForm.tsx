...
export const NativeSurveyForm: React.FC<Props> = ({ onSubmit, isLoading }) => {
    // Local State for all 54 fields
    const [form, setForm] = useState<any>({
        // 1-3 Profile
        checkupId: '', gender: '', department: [],
        // 4-14 History
    ...
    const handleSubmit = () => {
        ...
        const data: QuestionnaireData = {
            ...
            respiratory: {
                ...
                chronicPhlegm: false, // Only asked cough and phlegm in one question Q42? No, Q42 is cough/phlegm combined usually? Prompt says "Cough, phlegm?" -> Yes/No. 
                // Wait, Q42 text: "您是否在未感冒的情况下，经常咳嗽、咳痰？" -> Yes/No. 
                // I will set both if Yes.
            ...
            <div className="max-w-3xl mx-auto bg-white shadow-xl min-h-screen">
                ...
                <div className="p-8 space-y-8">
                    ...
                    <div className="space-y-6">
                        ...
                        <CheckQ idx={3} label="所在部门" required options={OPTIONS.department} value={form.department} onChange={v => toggleArray('department', v)} />
                    </div>
                    ...
                    {/* Section 2: History */}
                    <div className="space-y-6">
                        <CheckQ idx={4} label="既往病史" required options={OPTIONS.history} value={form.historyDiseases} onChange={v => toggleArray('historyDiseases', v)} />
                        
                        {form.historyDiseases.includes('高血压') && <InputQ idx={5} label="高血压诊断年份" required value={form.htnYear} onChange={v => handleChange('htnYear', v)} type="date" />}
                        {form.historyDiseases.includes('冠心病') && <CheckQ idx={6} label="冠心病类型" required options={OPTIONS.cadType} value={form.cadTypes} onChange={v => toggleArray('cadTypes', v)} />}
                        {form.historyDiseases.includes('心律失常') && <InputQ idx={7} label="心律失常类型" required value={form.arrhythmiaType} onChange={v => handleChange('arrhythmiaType', v)} />}
                        {form.historyDiseases.includes('脑卒中／短暂性脑缺血发作') && (
                            <>
                                <CheckQ idx={8} label="脑卒中类型" required options={OPTIONS.strokeType} value={form.strokeTypes} onChange={v => toggleArray('strokeTypes', v)} />
                                <InputQ idx={9} label="脑卒中发生年份" required value={form.strokeYear} onChange={v => handleChange('strokeYear', v)} type="date" />
                            </>
                        )}
                        {form.historyDiseases.includes('糖尿病／糖尿病前期') && <InputQ idx={10} label="糖尿病诊断年份" required value={form.dmYear} onChange={v => handleChange('dmYear', v)} type="date" />}
                        {form.historyDiseases.includes('肿瘤') && (
                            <>
                                <InputQ idx={11} label="肿瘤部位" required value={form.tumorSite} onChange={v => handleChange('tumorSite', v)} />
                                <InputQ idx={12} label="肿瘤诊断年份" required value={form.tumorYear} onChange={v => handleChange('tumorYear', v)} type="date" />
                            </>
                        )}
                        
                        <TextQ idx={13} label="手术史及外伤史" required desc="如：2010年，胆囊切除术；2015年，膝关节镜手术" value={form.surgeryHistory} onChange={v => handleChange('surgeryHistory', v)} />
                        <TextQ idx={14} label="其他既往病史" required value={form.otherHistory} onChange={v => handleChange('otherHistory', v)} />
                        
                        <RadioQ idx={15} label="是否规律服药" required options={['是', '否']} value={form.regularMeds} onChange={v => handleChange('regularMeds', v)} />
                        <CheckQ idx={16} label="您目前是否正在服用以下药物？" required options={OPTIONS.medication} value={form.medTypes} onChange={v => toggleArray('medTypes', v)} />
                        <InputQ idx={17} label="您服用的其他药物是？" required value={form.otherMedName} onChange={v => handleChange('otherMedName', v)} />
                        
                        <CheckQ idx={18} label="您的父母、兄弟姐妹（一级亲属）是否确诊过以下疾病？" required options={OPTIONS.family} value={form.familyHistory} onChange={v => toggleArray('familyHistory', v)} />
                        <RadioQ idx={19} label="父亲 - 冠心病/心肌梗死发病年龄是否<55岁？" required options={['是', '否']} value={form.fatherCvdEarly} onChange={v => handleChange('fatherCvdEarly', v)} />
                        <RadioQ idx={20} label="母亲 - 冠心病/心肌梗死发病年龄是否<65岁？" required options={['是', '否']} value={form.motherCvdEarly} onChange={v => handleChange('motherCvdEarly', v)} />
                    </div>
                    ...
                    {/* Section 3: Lifestyle */}
                    <div className="space-y-6">
                        <CheckQ idx={21} label="膳食习惯" required options={OPTIONS.diet} value={form.dietHabits} onChange={v => toggleArray('dietHabits', v)} />
                        <RadioQ idx={22} label="主食类型" required options={OPTIONS.staple} value={form.stapleType} onChange={v => handleChange('stapleType', v)} />
                        <RadioQ idx={23} label="粗粮杂豆根块类频率" required options={OPTIONS.coarseFreq} value={form.coarseFreq} onChange={v => handleChange('coarseFreq', v)} />
                        <RadioQ idx={24} label="蔬菜摄入" required options={OPTIONS.vegFruitMeat} value={form.vegIntake} onChange={v => handleChange('vegIntake', v)} />
                        <RadioQ idx={25} label="水果摄入" required options={['每天≥200克', '每天约100－200克', '摄入较少（＜100克）']} value={form.fruitIntake} onChange={v => handleChange('fruitIntake', v)} />
                        <RadioQ idx={26} label="肉蛋禽摄入" required options={['每天≥200克', '每天约100－200克', '摄入较少（＜100克）']} value={form.meatIntake} onChange={v => handleChange('meatIntake', v)} />
                        <RadioQ idx={27} label="奶制品摄入" required options={OPTIONS.dairy} value={form.dairyIntake} onChange={v => handleChange('dairyIntake', v)} />
                        <RadioQ idx={28} label="豆类及坚果摄入" required options={OPTIONS.beanNut} value={form.beanNutIntake} onChange={v => handleChange('beanNutIntake', v)} />
                        
                        <InputQ idx={29} label="每日饮水多少杯" required desc="500ml/杯，请填数字0-10" value={form.waterCups} onChange={v => handleChange('waterCups', v)} type="number" />
                        
                        <RadioQ idx={30} label="运动频率" required options={OPTIONS.exerciseFreq} value={form.exFreq} onChange={v => handleChange('exFreq', v)} />
                        <CheckQ idx={31} label="运动类型" required options={OPTIONS.exerciseType} value={form.exTypes} onChange={v => toggleArray('exTypes', v)} />
                        <RadioQ idx={32} label="平均每次运动时长" required options={OPTIONS.exerciseTime} value={form.exDuration} onChange={v => handleChange('exDuration', v)} />
                        
                        <InputQ idx={33} label="平均每晚睡眠几个小时" required desc="请填数字3-10" value={form.sleepHours} onChange={v => handleChange('sleepHours', v)} type="number" />
                        <RadioQ idx={34} label="睡眠质量" required options={OPTIONS.sleepQuality} value={form.sleepQuality} onChange={v => handleChange('sleepQuality', v)} />
                        <RadioQ idx={35} label="打鼾情况" required options={OPTIONS.snore} value={form.snore} onChange={v => handleChange('snore', v)} />
                        {form.snore !== '无' && (
                            <>
                                <RadioQ idx={36} label="打鼾是否做过睡眠监测" required options={['是', '否']} value={form.snoreMonitor} onChange={v => handleChange('snoreMonitor', v)} />
                                <InputQ idx={37} label="睡眠监测结果" required value={form.monitorResult} onChange={v => handleChange('monitorResult', v)} />
                            </>
                        )}

                        <RadioQ idx={38} label="吸烟情况" required options={OPTIONS.smoke} value={form.smokeStatus} onChange={v => handleChange('smokeStatus', v)} />
                        {form.smokeStatus === '已戒烟' && <InputQ idx={39} label="戒烟年份" required value={form.quitSmokeYear} onChange={v => handleChange('quitSmokeYear', v)} type="date" />}
                        {form.smokeStatus !== '从不吸烟' && (
                            <>
                                <InputQ idx={40} label="目前吸烟支数" required desc="每天支数" value={form.smokeDaily} onChange={v => handleChange('smokeDaily', v)} type="number" />
                                <InputQ idx={41} label="已吸烟年数" required value={form.smokeYears} onChange={v => handleChange('smokeYears', v)} type="number" />
                            </>
                        )}

                        <RadioQ idx={42} label="是否在未感冒的情况下，经常咳嗽、咳痰？" required options={['是', '否']} value={form.chronicCough} onChange={v => handleChange('chronicCough', v)} />
                        <RadioQ idx={43} label="是否在活动后比同龄人更容易气短？" required options={['是', '否']} value={form.shortBreath} onChange={v => handleChange('shortBreath', v)} />

                        <RadioQ idx={44} label="饮酒情况" required options={OPTIONS.drink} value={form.drinkStatus} onChange={v => handleChange('drinkStatus', v)} />
                        {form.drinkStatus === '目前饮酒' && (
                            <>
                                <InputQ idx={45} label="每周饮酒频次" required desc="次/周" value={form.drinkFreq} onChange={v => handleChange('drinkFreq', v)} type="number" />
                                <InputQ idx={46} label="每次饮酒量（几两）" required desc="两/次" value={form.drinkAmount} onChange={v => handleChange('drinkAmount', v)} type="number" />
                            </>
                        )}
                        <RadioQ idx={47} label="醉酒史 (过去12个月)" required options={OPTIONS.drunk} value={form.drunkHistory} onChange={v => handleChange('drunkHistory', v)} />
                        <RadioQ idx={48} label="戒酒意愿" required options={OPTIONS.quitDrink} value={form.quitDrinkIntent} onChange={v => handleChange('quitDrinkIntent', v)} />
                    </div>
                    ...
                    {/* Section 4: Mental */}
                    <div className="space-y-6">
                        <RadioQ idx={49} label="压力自我评估" required options={OPTIONS.stress} value={form.stressLevel} onChange={v => handleChange('stressLevel', v)} />
                        
                        {/* PHQ-9 */}
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                            <h3 className="font-bold text-slate-800 mb-2">50. 情绪状态 (PHQ-9) *</h3>
                        ...
                        {/* GAD-7 */}
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                            <h3 className="font-bold text-slate-800 mb-2">51. 焦虑状态 (GAD-7) *</h3>
                            <p className="text-xs text-slate-500 mb-4">请选择过去两周里，您生活中出现以下症状的频率。</p>
                            <div className="space-y-4">
                    ...
                    {/* Section 5: Needs */}
                    <div className="space-y-6">
                        <RadioQ idx={52} label="是否愿意接受医院定期开展健康随访服务" required options={OPTIONS.willing} value={form.followUpWilling} onChange={v => handleChange('followUpWilling', v)} />
                        <CheckQ idx={53} label="希望校医院提供的健康服务" required options={OPTIONS.services} value={form.desiredServices} onChange={v => toggleArray('desiredServices', v)} />
                        <TextQ idx={54} label="希望获得的其他健康支持" value={form.otherSupport} onChange={v => handleChange('otherSupport', v)} />
                    </div>
