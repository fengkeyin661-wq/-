...
export const UserLayout: React.FC<Props> = ({ activeTab, onTabChange, children, unreadCount = 0 }) => {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F2F2F7] max-w-md mx-auto shadow-2xl overflow-hidden relative font-sans">
      ...
      {/* 国际化主流 Tab Bar (Floating Pill with Glass Effect) */}
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${isActive ? 'text-slate-900 opacity-100' : 'text-slate-400 opacity-0'}`}>
                  {item.label}
                </span>
              </button>
            );
