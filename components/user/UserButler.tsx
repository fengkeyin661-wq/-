...
export const UserButler: React.FC<Props> = ({ record, assessment, userId, onClose, onNavigate }) => {
    const [messages, setMessages] = useState<Message[]>([
        { id: '1', role: 'model', text: `您好 ${record.profile.name}，我是您的私人健康助理。我已经根据您 ${record.profile.checkupDate || '近期'} 的体检数据（${assessment?.riskLevel === 'RED' ? '红色高风险' : '一般风险'}）为您定制了回复策略。请问您今天感觉如何？` }
    ]);
    ...
    const handleSend = async (textOverride?: string) => {
        ...
        // 准备上下文：包含档案关键信息和资源库
        const profileStr = `年龄:${record.profile.age}, 性别:${record.profile.gender}, 评估综述:${assessment?.summary}, 异常项:${[...(assessment?.risks.red || []), ...(assessment?.risks.yellow || [])].join('/')}`;
        ...
        try {
            ...
            setMessages(prev => [...prev, { id: (Date.now()+1).toString(), role: 'model', text: "抱歉，由于咨询人数较多，请稍后再试或直接咨询在线医生。" }]);
        } finally {
            setIsThinking(false);
        }
    };
    ...
    const toggleVoice = () => {
        ...
        if (!isListening) {
            ...
            setTimeout(() => {
                ...
                handleSend("我最近感觉膝盖不舒服，尿酸好像有点高，怎么调理？");
            }, 2500);
        }
    };
        ...
        <div className="fixed inset-0 bg-slate-100 z-[100] flex flex-col animate-slideUp">
            ...
            <div className="bg-white/80 backdrop-blur-xl border-b border-slate-200 px-5 py-4 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-black text-slate-800 text-sm">私人医生助理</h3>
                        <div className="flex items-center gap-1">
                            <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">DeepSeek Engine v3</span>
                        </div>
                    </div>
                </div>
                <button onClick={onClose} className="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-slate-900 font-bold bg-slate-50 rounded-full active:scale-90 transition-transform">×</button>
            </div>
                        ...
                        {msg.recommendations && msg.recommendations.length > 0 && (
                            <div className="mt-4 w-full space-y-3 animate-fadeIn">
                                <div className="flex items-center gap-2 pl-1">
                                    ...
                                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">智能管家为您匹配</p>
                                </div>
            ...
            <div className="p-4 bg-white border-t border-slate-100 shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
                {isListening && (
                    <div className="mb-4 flex flex-col items-center animate-fadeIn">
                        ...
                        <p className="text-[10px] text-indigo-600 font-black animate-pulse">正在倾听健康咨询...</p>
                    </div>
                )}
                ...
                <div className="flex gap-3 items-center">
                    ...
                    <div className="flex-1 bg-slate-100 rounded-2xl px-4 py-1 flex items-center border border-slate-200/50">
                        <input 
                            ...
                            placeholder={isListening ? "请说话..." : "输入症状、困惑或项目..."}
                            value={input}
                        ...
                        <button 
                            ...
                            className="w-9 h-9 bg-slate-900 text-white rounded-xl flex items-center justify-center shadow-lg active:scale-90 disabled:opacity-30 transition-all"
                        >
                            ➤
                        </button>
                    </div>
                ...
                <div className="mt-4 flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                     {["膝盖疼", "尿酸高", "预约张主任", "减重餐单"].map(tag => (
                         <button key={tag} onClick={() => handleSend(tag)} className="text-[10px] bg-white border border-slate-200 text-slate-500 px-3 py-1.5 rounded-full font-bold whitespace-nowrap active:bg-slate-100 transition-colors">
                             {tag}
                         </button>
                     ))}
                </div>
    ...
    return (
        <div className="snap-start min-w-[240px] max-w-[240px] bg-white rounded-[2rem] border border-slate-100 shadow-xl flex flex-col overflow-hidden group">
            <div className={`h-32 flex items-center justify-center text-5xl relative ${isDoc ? 'bg-blue-50' : isDrug ? 'bg-orange-50' : 'bg-emerald-50'}`}>
                {isDoc ? '👨‍⚕️' : isDrug ? '🍱' : '🏃'}
                <div className="absolute top-3 right-3 px-2 py-0.5 rounded-full bg-white/80 backdrop-blur-md text-[8px] font-black text-slate-500 uppercase">
                    {isDoc ? '专家' : isDrug ? '套餐' : '训练'}
                </div>
            </div>
            <div className="p-5 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-1">
                    <h4 className="font-black text-slate-800 text-sm truncate flex-1">{item.title}</h4>
                    {isDrug && <span className="text-orange-600 font-black text-xs ml-2">¥99+</span>}
                </div>
                
                <div className="space-y-1 mb-4 flex-1">
                    <p className="text-[10px] font-bold text-slate-400 line-clamp-2 leading-relaxed italic">
                        {item.description || '由校医院健康管理中心专家评审推荐'}
                    </p>
                    {isDoc && <p className="text-[9px] text-blue-600 font-black">擅长: {item.details?.dept || '全科医学'} · 慢病干预</p>}
                    {isEx && <p className="text-[9px] text-emerald-600 font-black">耗时: 15min · 燃脂 120kcal</p>}
                </div>
